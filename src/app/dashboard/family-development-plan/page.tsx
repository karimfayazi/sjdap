...
import { Download, Search, RefreshCw, Eye, X, FileText, Filter, Loader2, Users, Target, FileCheck } from "lucide-react";
...
function FamilyDevelopmentPlanPageContent() {
	...
	const fetchApplications = useCallback(async () => {
		try {
			...
			if (debouncedFilters.fromDate) params.append("fromDate", debouncedFilters.fromDate);
			if (debouncedFilters.toDate) params.append("toDate", debouncedFilters.toDate);
			if (debouncedFilters.approvalStatus) params.append("approvalStatus", debouncedFilters.approvalStatus);
			...
			if (data.success) {
				...
				// Extract filter options from data if available
				if (data.filterOptions) {
					setFilterOptions(data.filterOptions);
				} else {
					// Extract unique values from current data for filter options
					const regionalCommunities = [...new Set(data.data?.map((app: FamilyDevelopmentPlan) => app.RegionalCommunity).filter(Boolean) || [])].sort() as string[];
					const localCommunities = [...new Set(data.data?.map((app: FamilyDevelopmentPlan) => app.LocalCommunity).filter(Boolean) || [])].sort() as string[];
					const approvalStatuses = [...new Set(data.data?.map((app: FamilyDevelopmentPlan) => app.ApprovalStatus).filter(Boolean) || [])].sort() as string[];
					setFilterOptions({
						regionalCommunities,
						localCommunities,
						approvalStatuses,
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between bg-white rounded-xl shadow-md border border-gray-200 p-6">
				<div>
					...
					<p className="text-gray-600 mt-2">View and manage family development plans</p>
				</div>
			...
			<div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 space-y-4">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
					<div>
						...
						>
							<option value="">All</option>
							{filterOptions?.regionalCommunities.map((council) => (
					...
					<div>
						...
						>
							<option value="">All</option>
							{filterOptions?.localCommunities.map((council) => (
								<option key={council} value={council}>
									{council}
								</option>
							))}
						...
						<table className="min-w-full divide-y divide-gray-200">
							<thead className="bg-gray-50">
								<tr>
									...
									<th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Regional / Local</th>
									<th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Members</th>
									<th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Area</th>
									<th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Income</th>
									<th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Max Support</th>
									<th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
								</tr>
							</thead>
							<tbody className="bg-white divide-y divide-gray-200">
								{filteredApplications.map((app) => (
									<tr key={app.FormNumber} className="hover:bg-gray-50">
										<td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{app.FormNumber || "N/A"}</td>
										<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{app.Full_Name || "N/A"}</td>
										<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{app.CNICNumber || "N/A"}</td>
										<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
											{app.RegionalCommunity || "N/A"} / {app.LocalCommunity || "N/A"}
										</td>
										<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{app.TotalFamilyMembers !== null ? app.TotalFamilyMembers : 0}</td>
										<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{app.Area_Type || "N/A"}</td>
										<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{app.IncomeLevel || "N/A"}</td>
										<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">

...
function EditSWBFamilyContent() {
	...
	const [originalFamilyId, setOriginalFamilyId] = useState<string>("");
		...
		const fetchFamilyData = async () => {
			...
			try {
				...
				if (data.success && data.swbFamily) {
					const family = data.swbFamily;
					setOriginalCnic(family.CNIC || "");
					setOriginalFamilyId(family.FAMILY_ID || "");
					
					setFormData({
						CNIC: family.CNIC || "",
						Received_Application: family.Received_Application ? family.Received_Application.split('T')[0] : "",
						BTS_Number: family.BTS_Number || "",
						FAMILY_ID: family.FAMILY_ID || "",
						Regional_Council: family.Regional_Council || "",
						Local_Council: family.Local_Council || "",
						Jamat_Khana: family.Jamat_Khana || "",
						Programme: family.Programme || "",
						Beneficiary_Name: family.Beneficiary_Name || "",
						Gender: family.Gender || "",
						VIST_FEAP: family.VIST_FEAP || "",
						Already_FEAP_Programme: family.Already_FEAP_Programme || "",
						Potential_family_declaration_by_FEAP: family.Potential_family_declaration_by_FEAP || "",
						If_no_reason: family.If_no_reason || "",
						FDP_Status: family.FDP_Status || "",
						SWB_to_stop_support_from_date: family.SWB_to_stop_support_from_date ? family.SWB_to_stop_support_from_date.split('T')[0] : "",
						Remarks: family.Remarks || "",
						Mentor_Name: family.Mentor_Name || "",
						Social_Support_Amount: family.Social_Support_Amount ? String(family.Social_Support_Amount) : "",
						Economic_Support_Amount: family.Economic_Support_Amount ? String(family.Economic_Support_Amount) : "",
					});

					// Fetch family head if FDP Status is Approved
					if (family.FDP_Status === "Approved" && family.FAMILY_ID) {
						const headResponse = await fetch(`/api/swb-families?getFamilyHead=true&familyId=${encodeURIComponent(family.FAMILY_ID)}`);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (data.success) {
				...
				setError(data.message || "Failed to update SWB family record");
			}
		} catch (err) {
			console.error("Error updating SWB family record:", err);
			setError("Error updating SWB family record");
		} finally {
			setSaving(false);
		}
	};
		...
		return (
			<div className="space-y-6">
				<div className="flex items-center justify-between">
					<div>
						<h1 className="text-3xl font-bold text-gray-900">Edit SWB Family Record</h1>
	...
	return (
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-3xl font-bold text-gray-900">Edit SWB Family Record</h1>
					<p className="text-gray-600 mt-2">Update SWB family record</p>
				</div>
				<button
			...
			{success && (
				<div className="bg-green-50 border border-green-200 rounded-lg p-4">
					<p className="text-green-700 text-sm">SWB family record updated successfully! Redirecting...</p>
				</div>
			)}
...
export default function EditSWBFamilyPage() {
	return (
		<Suspense fallback={
			<div className="space-y-6">
				<div className="flex items-center justify-between">

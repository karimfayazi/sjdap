...
export async function GET(request: NextRequest) {
	try {
		...
		// CRITICAL: Filter to only families with Open + Approved interventions
		whereConditions.push(`EXISTS (
			SELECT 1 
			FROM [SJDA_Users].[dbo].[PE_Interventions] i 
			WHERE i.[FormNumber] = b.[FormNumber] 
			AND LOWER(LTRIM(RTRIM(i.[InterventionStatus]))) = 'open'
			AND LOWER(LTRIM(RTRIM(i.[ApprovalStatus]))) = 'approved'
		)`);
		...
		// Status filter (from PE_Interventions) - Note: This is redundant since we already filter by Open, but kept for consistency
		if (status && status !== "All") {
			sqlRequest.input("status", sql.NVarChar, status);
			whereConditions.push(`EXISTS (
				SELECT 1 FROM [SJDA_Users].[dbo].[PE_Interventions] i 
				WHERE i.[FormNumber] = b.[FormNumber] 
				AND LOWER(LTRIM(RTRIM(i.[InterventionStatus]))) = LOWER(LTRIM(RTRIM(@status)))
				AND LOWER(LTRIM(RTRIM(i.[ApprovalStatus]))) = 'approved'
			)`);
		}
		...
		const query = `
			...
			LEFT JOIN (
				...
				WHERE LOWER(LTRIM(RTRIM([ApprovalStatus]))) = 'approved'
		...
		console.error("Error fetching ROPS families data:", error);

		const errorMessage = error instanceof Error ? error.message : String(error);
		return NextResponse.json(
			{
				success: false,
				message: "Error fetching ROPS families data: " + errorMessage,
				records: [],
			},
			{ status: 500 }
		);
